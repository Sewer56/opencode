...
import CODESEARCH_DESC from "../src/tool/codesearch.txt"
...
import { Parameters as CodesearchParams } from "../src/tool/codesearch"
...
const toolMetadata: Record<string, { description: string; parameters: any }> = {
  bash: { description: applyBashTemplate(BASH_DESC), parameters: BashParams },
  ...
  codesearch: { description: CODESEARCH_DESC, parameters: CodesearchParams },
...
function applyBashTemplate(desc: string): string {
  // These are the dynamic values that bash tool injects at runtime
  return desc
